...
// src/git/runtime/git-wrapper.mjs
var REAL_GIT = process.env.GIT_COORDINATOR_REAL_GIT || process.env.COORDINATED_GIT_REAL || "/usr/bin/git";
var INTERNAL_ENVIRONMENT_VARIABLE = "GIT_COORDINATOR_INTERNAL";
var LEGACY_INTERNAL_ENVIRONMENT_VARIABLE = "COORDINATED_GIT_INTERNAL";
var PINNED_RESOLUTION_ENVIRONMENT_VARIABLE = "AGENT_COORDINATOR_PINNED_RESOLUTION";
var SUPPORTED_COMMANDS = /* @__PURE__ */ new Set([
  "add",
  "checkout",
  "commit",
  "pull",
...
var CoordinatedGitError = class extends Error {
...
function parseInvocation(argumentsList, initialDirectory) {
  ...
  while (index < argumentsList.length) {
    ...
    if (argument.startsWith("-c") || argument.startsWith("--config-env=") || argument === "--no-pager" || argument === "--paginate" || argument === "--literal-pathspecs" || argument === "--glob-pathspecs" || argument === "--noglob-pathspecs" || argument === "--icase-pathspecs") {
      forwardedGlobalOptions.push(argument);
      index += 1;
      continue;
    }
    if (argument === "--git-dir" || argument === "--work-tree" || argument === "--namespace") {
      return null;
    }
...
function normalizeBranchPolicy(value, label) {
  ...
  if (mode === "map") {
    if (!value.branches || typeof value.branches !== "object" || Array.isArray(value.branches) || Object.keys(value.branches).length === 0) {
      throw new CoordinatedGitError(
...
function normalizeWorkspaceManifest(value, label, rootDirectory) {
  ...
  if (typeof manifestPath !== "string" || manifestPath.length === 0 || path.isAbsolute(manifestPath) || manifestPath.split(/[\\/]/).includes("..")) {
  ...
  const mirrorActiveInLinkedWorktrees = value.mirrorActiveInLinkedWorktrees ?? false;
...
function configurationReference(relativePath, source) {
  ...
  return source === "index" ? `:${relativePath}` : `${source.revision}:${relativePath}`;
}
function configurationFile(rootDirectory, relativePath, source) {
  ...
  return shown.status === 0 ? { contents: shown.stdout, label: revision } : null;
}
function loadContext(invocation, configurationSource = "worktree") {
  ...
  if (![1, 2].includes(configuration?.schemaVersion) || !Array.isArray(configuration.repositories) || configuration.repositories.length === 0) {
  ...
  if (configuration.workspace !== void 0 && configuration.workspaceManifest !== void 0) {
    throw new CoordinatedGitError(
      `${configurationLabel} cannot contain both workspace and workspaceManifest.`
    );
  }
  const workspaceManifest = configuration.workspace !== void 0 ? normalizeInlineWorkspace(
    configuration.workspace,
    `${configurationLabel}.workspace`,
    rootDirectory
  ) : configurationLabel.endsWith("coordinator.yaml") || configuration.schemaVersion === 2 ? normalizeWorkspaceManifest(
  ...
  const repositories = configuration.repositories.map((entry, index) => {
    if (typeof entry?.id !== "string" || typeof entry?.path !== "string" || entry.path.length === 0 || path.isAbsolute(entry.path) || entry.path.split(/[\\/]/).includes("..")) {
      throw new CoordinatedGitError(
        `${configurationLabel} contains an invalid repository entry.`
      );
    }
    const branchPolicy = configuration.schemaVersion === 1 && !configurationLabel.endsWith("coordinator.yaml") ? { mode: "mirror", readOnly: false } : normalizeBranchPolicy(
      entry.branch,
  ...
  if (new Set(repositories.map(({ path: repositoryPath }) => repositoryPath)).size !== repositories.length) {
    throw new CoordinatedGitError(
...
function isRepositoryAt(directory) {
  ...
  return result.status === 0 && canonicalPath(result.stdout) === canonicalPath(directory);
}
...
function isLinkedWorktree(rootDirectory) {
  ...
  const resolveGitPath = (value) => canonicalPath(path.resolve(rootDirectory, value));
...
function parseExternalWorkspaceManifest(context, source, label) {
  ...
  if (!parsed || typeof parsed !== "object" || Array.isArray(parsed) || !parsed.repositories || typeof parsed.repositories !== "object" || Array.isArray(parsed.repositories)) {
    throw new CoordinatedGitError(
  ...
  if (parsed.baseBranch === context.workspaceManifest.coordinatorToken || typeof parsed.baseBranch !== "string") {
    throw new CoordinatedGitError(
  ...
  const expectedRepositoryIds = context.repositories.map((repository) => repository.id).sort();
  if (actualRepositoryIds.length !== expectedRepositoryIds.length || actualRepositoryIds.some(
...
function assertInlineWorkspaceTopology(context, configuration, label) {
  ...
  for (const entry of configuration.repositories) {
    if (typeof entry?.id !== "string" || typeof entry?.path !== "string" || actual.has(entry.id)) {
      throw new CoordinatedGitError(
  ...
  if (actual.size !== context.repositories.length || context.repositories.some(
...
function parseInlineWorkspaceManifest(context, configuration, label) {
  ...
  if (!workspace || typeof workspace !== "object" || Array.isArray(workspace) || !workspace.selection || typeof workspace.selection !== "object" || Array.isArray(workspace.selection)) {
    throw new CoordinatedGitError(
      `${label}.workspace must contain a selection object.`
    );
  }
  if (workspace.baseBranch === context.workspaceManifest.coordinatorToken || typeof workspace.baseBranch !== "string") {
    throw new CoordinatedGitError(
  ...
  const mirrorActiveInLinkedWorktrees = workspace.mirrorActiveInLinkedWorktrees ?? false;
  if (coordinatorToken !== context.workspaceManifest.coordinatorToken || mirrorActiveInLinkedWorktrees !== context.workspaceManifest.mirrorActiveInLinkedWorktrees) {
    throw new CoordinatedGitError(
  ...
  for (const repository of context.repositories) {
    ...
    if (!entry || typeof entry !== "object" || Array.isArray(entry) || Object.keys(entry).sort().join(",") !== "branch,mode" || typeof entry.branch !== "string" || !["active", "pinned"].includes(entry.mode)) {
      throw new CoordinatedGitError(
...
function workspaceSource(context, source) {
  ...
  const revision = source === "index" ? `:${manifestPath}` : `${source.revision}:${manifestPath}`;
...
function manifestPolicyContextFromValue(context, coordinatorBranch, manifest) {
  ...
  const repositories = context.repositories.map((repository) => {
    ...
    if (!entry || typeof entry !== "object" || Array.isArray(entry) || Object.keys(entry).sort().join(",") !== "branch,mode,path" || entry.path !== repository.path || typeof entry.branch !== "string" || !["active", "pinned"].includes(entry.mode)) {
      throw new CoordinatedGitError(
        `${context.workspaceManifest.path} contains an invalid entry for ${repository.id}.`
      );
    }
    const resolvedBranch = entry.mode === "active" && mirrorActive ? coordinatorBranch : entry.branch === context.workspaceManifest.coordinatorToken ? coordinatorBranch : configuredBranchName(
      entry.branch,
      `${context.workspaceManifest.path}.repositories.${repository.id}.branch`
    );
    return {
      ...repository,
      branchPolicy: entry.mode === "pinned" ? {
        mode: "pinned",
        name: resolvedBranch,
        readOnly: true,
...
function creationManifest(context, coordinatorBranch) {
  ...
  for (const repository of configured.repositories) {
    ...
    const branch = policy.mode === "mirror" ? context.workspaceManifest.coordinatorToken : resolvedRepositoryBranch(repository, coordinatorBranch);
...
function rootGitlink(context, repository, rootReference = null) {
  const revision = rootReference ? `${rootReference}:${repository.path}` : `:${repository.path}`;
...
function branchContainsRevision(context, repository, branch, revision) {
  ...
  return [
    ...
    return exists.status === 0 && gitText(
      repository.directory,
...
function assertBranchInvariant(context, rootReference = null) {
  ...
  for (const repository of context.repositories) {
    ...
    if (repository.branchPolicy.mode === "pinned" && !branch) {
      ...
      if (gitlink.status === 0 && branchContainsRevision(
        context,
        repository,
        expectedBranch,
        gitlink.stdout
  ...
  if (mismatches.length > 0) {
    const details = mismatches.map(
      ({ repository, branch, expectedBranch }) => `${repository.id}=${branch ?? "DETACHED"} (expected ${expectedBranch})`
    ).join(", ");
    throw new CoordinatedGitError(
...
function isBroadAdd(options, pathspecs) {
  ...
  return options.some(
    (option) => ["-A", "--all", "-u", "--update"].includes(option)
  );
}
function coordinatedAdd(context) {
  ...
  for (const pathspec of pathspecs) {
    ...
    const childPath = normalized === repository.path ? "." : normalized.slice(repository.path.length + 1);
...
function hasStagedChanges(repository) {
  return gitText(repository, ["diff", "--cached", "--quiet"], {
    allowFailure: true
  }).status !== 0;
}
function hasTrackedWorktreeChanges(repository) {
  return gitText(repository, ["diff", "--quiet"], { allowFailure: true }).status !== 0;
}
...
function hasExplicitCommitMessage(argumentsList) {
  return argumentsList.some(
    (argument) => argument === "-m" || argument.startsWith("-m") || argument === "--message" || argument.startsWith("--message=") || argument === "-F" || argument.startsWith("-F") || argument === "--file" || argument.startsWith("--file=")
  );
}
function childCommitArguments(rootDirectory, argumentsList) {
  ...
  for (let index = 0; index < argumentsList.length; index += 1) {
    ...
    if (argument === "-F" || argument === "--file") {
      ...
      normalized.push(
        argument,
        value === "-" || path.isAbsolute(value) ? value : path.resolve(rootDirectory, value)
      );
      index += 1;
      continue;
    }
...
function repositoryHasCommitCandidate(repository, argumentsList) {
  ...
  return commitIncludesAllTrackedChanges(argumentsList) && hasTrackedWorktreeChanges(repository);
}
...
function coordinatedCommit(context) {
  ...
  assertBranchInvariant(context);
  assertReadOnlyRepositoriesClean(context);
  const candidates = writableRepositories(context).filter(
    (repository) => repositoryHasCommitCandidate(repository.directory, context.commandArguments)
  );
...
function pullArguments(argumentsList) {
  const allowedOptions = /* @__PURE__ */ new Set([
    "--ff-only",
    "--no-rebase",
    "--no-progress",
    "--progress",
...
`
  ...
  const result = target.root ? executeRootGit(context, argumentsList) : executeGit(target.directory, argumentsList);
  if (result.status !== 0) {
    throw new CoordinatedGitError(
...
function coordinatedPull(context) {
  ...
  const remote = positionals[0] || configuredRemote.stdout || context.configuration.remote || "origin";
  const requestedBranch = positionals[1];
  if (requestedBranch && ![branch, `refs/heads/${branch}`].includes(requestedBranch)) {
    throw new CoordinatedGitError(
  ...
  const diverged = plans.filter((plan) => plan.state === "diverged" && !plan.target.readOnly);
  if (diverged.length > 0) {
    throw new CoordinatedGitError(
      `coordinated pull cannot fast-forward: ${diverged.map((plan) => `${plan.target.label}/${plan.target.branch}`).join(", ")}. Resolve the divergence explicitly before retrying.`,
      "DIVERGED_HISTORY"
    );
  }
  ...
  if (incomingContext.repositories.length !== context.repositories.length || incomingContext.repositories.some((repository) => {
    const previous = context.repositories.find((entry) => entry.id === repository.id);
    return !previous || previous.path !== repository.path || previous.branchPolicy.readOnly !== repository.branchPolicy.readOnly || resolvedRepositoryBranch(previous, branch) !== resolvedRepositoryBranch(repository, branch);
  })) {
    throw new CoordinatedGitError(
  ...
  const incomingRevisions = /* @__PURE__ */ new Map();
  for (const repository of incomingContext.repositories) {
    ...
    if (!childPlan || !revisionIsAncestor(repository, revision, childPlan.remoteRevision)) {
      throw new CoordinatedGitError(
        repository.id + " cannot obtain incoming gitlink " + revision.slice(0, 8) + " from " + remote + "/" + resolvedRepositoryBranch(repository, branch) + ". The remote coordinator references an unpublished or rewritten commit. Repair that gitlink or restore the commit before retrying. No worktree or local branch was moved.",
        "MISSING_INCOMING_COMMIT"
      );
    }
    if (repository.branchPolicy.readOnly && repository.branchPolicy.mode !== "pinned" && revision !== childPlan.remoteRevision && revision !== gitText(repository.directory, ["rev-parse", "HEAD"]).stdout) {
      throw new CoordinatedGitError(
    ...
    if (repository.branchPolicy.readOnly && repository.branchPolicy.mode !== "pinned" && revision !== gitText(repository.directory, ["rev-parse", "HEAD"]).stdout && childPlan.state !== "behind") {
      throw new CoordinatedGitError(
  ...
  for (const plan of applyPlans) {
    ...
    if (plan.target.readOnly && currentRevision === incomingRevisions.get(plan.target.repository.id)) {
      continue;
    }
...
`
    ...
    const result = plan.target.root ? executeRootGit(context, argumentsList) : executeGit(plan.target.directory, argumentsList);
    if (result.status !== 0) {
      throw new CoordinatedGitError(
...
function pushWritableRepositories(context, branch, remote, options = []) {
  ...
  for (const repository of writableRepositories(context)) {
    ...
    const childOptions = childUpstream.status !== 0 ? [...options, "--set-upstream"] : options;
    process.stderr.write(
...
`
    ...
    if (result.status !== 0) {
      const completed = published.length > 0 ? ` Already pushed: ${published.join(", ")}.` : "";
      process.stderr.write(
...
function coordinatedPush(context) {
  ...
  const automaticallySetUpstream = positionals.length === 0 && upstream.status !== 0;
...
`);
  const rootPushArguments = automaticallySetUpstream ? ["push", "--set-upstream", remote, `HEAD:refs/heads/${branch}`] : ["push", ...context.commandArguments];
...
function restoreRepository(repository, state) {
  const argumentsList = state.branch ? ["switch", state.branch] : ["switch", "--detach", state.revision];
...
function prepareBranchAtRevision(repository, branch, desiredRevision, createdBranches) {
...
function assertPreparedBranchRevision(repository, prepared, { checkedOut = false } = {}) {
  ...
  const checkoutMatches = !checkedOut || currentBranch(repository.directory) === prepared.branch && gitText(repository.directory, ["rev-parse", "HEAD"]).stdout === prepared.expectedBranchRevision;
  if (reference.status !== 0 || reference.stdout !== prepared.expectedBranchRevision || !checkoutMatches) {
    throw new CoordinatedGitError(
...
function pinnedBranchResolution(repository, branch, pinnedRevision, branchRevision) {
...
function fetchLatestPinnedRevision(context, repository, branch, pinnedRevision) {
  const remote = context.configuration.remote || "origin";
  const temporaryReference = `refs/agent-coordinator/pinned-resolution/${randomUUID()}`;
  process.stderr.write(
...
`
  ...
  try {
    ...
    if (cleanup.status !== 0) {
      const cleanupMessage = `could not remove temporary ref ${temporaryReference} in ${repository.id}.`;
      if (operationError) {
        throw new CoordinatedGitError(
...
function prepareRepositoryAtRevision(context, repository, coordinatorBranch, desiredRevision, createdBranches, options = {}) {
  ...
  if (repository.branchPolicy.mode !== "pinned") {
    ...
    if (options.reconcileExistingBranch && branchExists(repository.directory, repositoryBranch)) {
      assertCommitAvailable(repository, desiredRevision, "the target branch");
      const branchRevision2 = gitText(repository.directory, [
        "rev-parse",
        `refs/heads/${repositoryBranch}`
      ]).stdout;
      if (branchRevision2 === desiredRevision) {
        return {
          branch: repositoryBranch,
          created: false,
          desiredRevision,
          detached: false,
          expectedBranchRevision: branchRevision2,
          updateGitlink: false
        };
      }
      if (!repository.branchPolicy.readOnly && revisionIsAncestor(repository, desiredRevision, branchRevision2)) {
        return {
          branch: repositoryBranch,
          created: false,
          desiredRevision: branchRevision2,
          detached: false,
          expectedBranchRevision: branchRevision2,
      ...
      if (!repository.branchPolicy.readOnly) {
        if (revisionIsAncestor(repository, branchRevision2, desiredRevision)) {
          throw new CoordinatedGitError(
            `${repository.id} branch '${repositoryBranch}' at ${branchRevision2.slice(0, 8)} is behind target gitlink ${desiredRevision.slice(0, 8)}. Fast-forward the child branch before switching.`
          );
        }
        throw new CoordinatedGitError(
          `${repository.id} branch '${repositoryBranch}' at ${branchRevision2.slice(0, 8)} has diverged from target gitlink ${desiredRevision.slice(0, 8)}. Resolve the child branch or mapping before switching.`
        );
      }
    }
    const created = options.planOnly ? planBranchAtRevision(repository, repositoryBranch, desiredRevision) : prepareBranchAtRevision(
      repository,
      repositoryBranch,
      desiredRevision,
      createdBranches
  ...
  if (checkedOutBranch === repositoryBranch && checkedOutRevision === desiredRevision) {
    return {
      branch: repositoryBranch,
      created: false,
      desiredRevision,
...
function assertCommitAvailable(repository, revision, requirement = "the start-point") {
  const result = gitText(
    repository.directory,
...
function planRepositoryAtStartPoint(context, repository, coordinatorBranch, desiredRevision) {
...
function fileState(file) {
  return existsSync(file) ? { contents: readFileSync(file, "utf8"), exists: true } : { exists: false };
}
function restoreFileState(file, state) {
  if (state.exists) {
    writeFileSync(file, state.contents, { mode: 420 });
...
function createCoordinatedBranchAtStartPoint(context, branch, startPoint) {
  ...
  const targetContext = targetConfiguration.workspaceManifest ? manifestPolicyContext(targetConfiguration, branch, {
  ...
  const nextManifest = targetConfiguration.workspaceManifest ? creationManifestFromValue(
    targetConfiguration,
    branch,
    targetContext.workspaceManifestValue
  ) : null;
  const manifestState = targetConfiguration.workspaceManifest ? fileState(targetConfiguration.workspaceManifest.absolutePath) : null;
  const states = [
...
function switchCoordinatedBranch(context, branch) {
  ...
  const targetContext = context.workspaceManifest ? manifestPolicyContext(context, branch, { revision: branch }) : context;
  let preparedRepositories = [];
  try {
...
function coordinatedCheckout(context) {
  ...
  const createFlags = context.command === "checkout" ? /* @__PURE__ */ new Set(["-b"]) : /* @__PURE__ */ new Set(["-c"]);
  const forceCreateFlags = context.command === "checkout" ? /* @__PURE__ */ new Set(["-B"]) : /* @__PURE__ */ new Set(["-C"]);
  const createFlagIndex = argumentsList.findIndex(
    (argument) => createFlags.has(argument)
  );
  const forceCreateFlagIndex = argumentsList.findIndex(
    (argument) => forceCreateFlags.has(argument)
  );
  ...
  if (createFlagIndex >= 0) {
    const branch2 = argumentsList[createFlagIndex + 1];
    const startPoint = argumentsList[createFlagIndex + 2];
    if (createFlagIndex !== 0 || !branch2 || argumentsList.length < 2 || argumentsList.length > 3) {
      throw new CoordinatedGitError(
    ...
    if (startPoint) {
      return createCoordinatedBranchAtStartPoint(context, branch2, startPoint);
    }
    return createCoordinatedBranch(context, branch2);
  }
  ...
  if (!branchExists(context.rootDirectory, branch)) {
    if (context.command === "checkout" && (existsSync(path.join(context.rootDirectory, branch)) || gitText(
      context.rootDirectory,
...
function sanitizedWorktreeBranch(worktreeDirectory) {
  const slug = path.basename(worktreeDirectory).replace(/[^A-Za-z0-9._-]+/g, "-").replace(/^-+|-+$/g, "") || "worktree";
  return `codex/${slug}`;
}
...
function prePushReferences() {
  ...
  return input.trim().split("\n").filter(Boolean).map((line) => {
    ...
    const [localReference, localRevision, remoteReference, remoteRevision] = fields;
    return {
      localReference,
      localRevision,
      remoteReference,
...
function assertDirectPushReference(context, branch, remote, reference) {
  ...
  if (reference.localRevision !== head || ![
    expectedLocalReference,
    "HEAD",
    reference.localRevision
  ].includes(reference.localReference) || reference.remoteReference !== expectedRemoteReference) {
    throw new CoordinatedGitError(
...
function runHook(argumentsList) {
  ...
  const configurationSource = hook === "pre-push" ? { revision: "HEAD" } : hook === "pre-commit" ? "index" : "worktree";
  const context = loadContext(invocation, configurationSource);
  if (!context) return 0;
  if (hook === "post-checkout") {
    if (hookArguments[2] !== "1") return 0;
    if (process.env[INTERNAL_ENVIRONMENT_VARIABLE] === "1" || process.env[LEGACY_INTERNAL_ENVIRONMENT_VARIABLE] === "1") {
      return 0;
    }
  ...
  if (hook === "pre-push") {
    const internal = process.env[INTERNAL_ENVIRONMENT_VARIABLE] === "1" || process.env[LEGACY_INTERNAL_ENVIRONMENT_VARIABLE] === "1";
  ...
  if (hook === "pre-commit") {
    ...
    const source = context.workspaceManifest && hook === "pre-commit" ? "index" : context.workspaceManifest ? { revision: "HEAD" } : "worktree";
...
function recoveryChanges(directory, ignoredPaths = []) {
  ...
  for (let index = 0; index < entries.length; index++) {
    ...
    const managedGitlinkChange = ignoredPaths.includes(file) && [" M", "M ", "MM"].includes(status);
...
function lastFailurePath(context) {
  return path.resolve(context.rootDirectory, gitText(context.rootDirectory, [
    "rev-parse",
    "--git-path",
    "agent-coordinator-last-error.json"
  ]).stdout);
}
...
function diagnoseRecovery(context) {
  ...
  const recordSteps = [];
  const repositories = [];
  const addIssue = (code, repository, message, commands = []) => issues.push({ code, repository, message, commands });
  ...
  for (const repository of context.repositories) {
    ...
    const pinnedDetached = repository.branchPolicy.mode === "pinned" && !state.branch && gitlink.status === 0 && expectedBranch && branchContainsRevision(context, repository, expectedBranch, gitlink.stdout);
    const branchMatches = Boolean(expectedBranch && (state.branch === expectedBranch || pinnedDetached));
    const reference = expectedBranch ? gitText(repository.directory, ["rev-parse", "--verify", "refs/heads/" + expectedBranch], { allowFailure: true }).stdout : null;
    repositories.push({
      id: repository.id,
      path: repository.path,
      initialized: true,
      ...
      expectedBranchRevision: reference,
      readOnly: repository.branchPolicy.readOnly,
      changes,
      entry
    });
    ...
    const upstream2 = gitText(repository.directory, ["rev-parse", "--verify", "@{upstream}"], { allowFailure: true });
    if (upstream2.status === 0 && !revisionIsAncestor(repository, state.revision, upstream2.stdout) && !revisionIsAncestor(repository, upstream2.stdout, state.revision)) {
  ...
  if (upstream.status === 0) {
    for (const repository of context.repositories) {
      ...
      if (incoming.status === 0 && gitText(repository.directory, ["cat-file", "-e", incoming.stdout + "^{commit}"], { allowFailure: true }).status !== 0) {
        addIssue(
          "MISSING_INCOMING_COMMIT",
          repository.id,
  ...
  const snapshot = {
    branch,
    head: gitText(context.rootDirectory, ["rev-parse", "HEAD"]).stdout,
    rootChanges,
    repositories,
    plans,
  ...
  return {
    ...
    plans,
    repositories,
    lastFailure: readLastFailure(context),
    applied: false
  };
}
function applyRecovery(context, strategy, snapshot) {
  ...
  if (report.snapshot !== snapshot) throw new CoordinatedGitError(
    "The workspace changed since this preview. Run coordinator git recover again to review a fresh plan.",
    "STALE_RECOVERY_PLAN"
  );
  const plan = report.plans.find((entry) => entry.strategy === strategy);
  if (!plan?.available) throw new CoordinatedGitError(
    "This recovery cannot be applied: " + (plan?.blocked.join(" ") || "no changes to apply."),
    "RECOVERY_BLOCKED"
  );
  const states = context.repositories.map((repository) => ({
    repository: repository.directory,
  ...
  try {
    ...
    assertReadOnlyRepositoriesClean(context);
    return {
      ...diagnoseRecovery(context),
      applied: true,
      strategy,
      next: strategy === "record" ? ["git diff --cached --submodule=short", "git commit -m 'Record coordinated repository revisions'"] : ["coordinator git check", "Retry your original Git command."]
    };
  } catch (error) {
...
function main() {
  ...
  if (argumentsList[0] === "--diagnose" || argumentsList[0] === "--recover") {
    ...
    const context2 = loadContext(invocation2);
    if (!context2) throw new CoordinatedGitError(
    ...
    if (argumentsList[0] === "--diagnose") {
      process.stdout.write(JSON.stringify(diagnoseRecovery(context2)) + "\n");
      return 0;
    }
    ...
    if (!["align", "record"].includes(strategy) || !snapshot) {
      throw new CoordinatedGitError(
        "Recovery requires a reviewed strategy and snapshot.",
        "INVALID_RECOVERY_REQUEST"
      );
    }
    process.stdout.write(JSON.stringify(applyRecovery(context2, strategy, snapshot)) + "\n");
    return 0;
  }
  ...
  if (argumentsList[0] === "--check") {
    ...
    const effectiveContext = currentPolicyContext(context2);
...
`
  ...
  if (argumentsList[0] === "--attach") {
    ...
    initializeMissingSubmodules(context2);
    return attachCoordinatedBranches(context2);
  }
  if (process.env[INTERNAL_ENVIRONMENT_VARIABLE] === "1" || process.env[LEGACY_INTERNAL_ENVIRONMENT_VARIABLE] === "1") {
...
try {
  ...
  const message = error instanceof Error ? error.message : "unknown coordinated Git error";
  process.stderr.write(
